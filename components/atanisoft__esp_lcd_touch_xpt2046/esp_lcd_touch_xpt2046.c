...
static float _gX_Bias = 0.0;
static float _gY_Bias = 0.0;
static float _gDeltaX= 0.0;
static float _gDeltaY= 0.0;
static uint16_t  _screenHor=0;
static uint16_t  _screenVer=0;
                                    ...
                                    float y_max)
{
    if (config == NULL)
    {
     ...
     _screenHor=config->x_max;
     _screenVer=config->y_max;
     
    _gDeltaX = (x_max - x_min);
    _gDeltaY = (y_max - y_min);
    _gX_Bias = x_min / _gDeltaX;
    _gY_Bias = y_min / _gDeltaY;
                                        ...
                                        esp_lcd_touch_handle_t *out_touch)
{
    ...
    ESP_LOGD(TAG, "esp_lcd_touch_new_spi_xpt2046 ----------------------------- init");
    ...
    if (z >= CONFIG_XPT2046_Z_THRESHOLD)
    {
        ...
        for (uint8_t idx = 0; idx < CONFIG_ESP_LCD_TOUCH_MAX_POINTS; idx++)
        {
            ...
            if ((x_temp >= 50) && (x_temp <= XPT2046_ADC_LIMIT - 50) && (y_temp >= 50) && (y_temp <= XPT2046_ADC_LIMIT - 50))
            {
                ...
                // coordinates.
                x_temp= (x_temp/_gDeltaX- _gX_Bias)* _screenHor;
                y_temp= (y_temp/_gDeltaY- _gY_Bias)* _screenVer;
                x += x_temp;
                y += y_temp;    
